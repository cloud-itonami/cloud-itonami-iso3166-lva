(ns marketentry.marketentryllm
  ...
  the Latvia public-sector market-entry compliance actor.

  It normalizes engagement intake, drafts a per-jurisdiction market-
  entry evidence checklist (including the EIS platform-operator
  citation -- see `marketentry.facts`/`marketentry.governor`), drafts
  the filing-draft action, and drafts the filing-submit action.
  CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record or a real portal submission. Every output is
  censored downstream by `marketentry.governor` before anything
...
(defn- platform-operator-flags
  ...
  the naive 'IUB operates EIS' mistake that collapses the legal/
  regulatory-oversight authority and the platform's separate technical
  operator into one fused value."
...
(defn- assess-jurisdiction
  ...
  vertical must defend against: fusing EIS's legal-oversight authority
  (IUB) and its separate technical operator (VDAA) into one claim."
    ...
    (if (nil? sb)
      ...
      {:summary    (str iso3 " (" (:owner-authority sb) ") 向け必要書類 "
       ...
       :rationale  (str "公式ソース: " (:provenance sb) " / 法的根拠: " (:legal-basis sb)
                        (when pob
                          (str " / EISプラットフォーム運営: " (:platform-operator-authority pob)
                               " (監督機関: " (:platform-oversight-authority pob) " とは別主体)")))
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ur-registration?=" (:has-ur-registration? e)
                        " vat-registration-verified?=" (:vat-registration-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ur-registration? e))
                              (:has-ur-registration? e))
                          (or (not (:requires-vat-registration? e))
                              (:vat-registration-verified? e)))
                   0.9 0.3)}))
