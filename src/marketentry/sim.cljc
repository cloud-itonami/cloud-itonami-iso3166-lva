(ns marketentry.sim
  ...
  through intake -> jurisdiction assessment (EIS platform-operator
  claim correctly distinguishes IUB from VDAA) -> filing draft
  (escalate/approve/commit) -> filing submit (escalate/approve/
  commit), then shows HARD-hold scenarios, including the
  platform-operator-fusion flagship check."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (LVA, clean) ==")
    (println (exec-op actor "t1" {:op :engagement/intake :subject "eng-1"
                                  :patch {:id "eng-1" :operator "Kita Systems SIA"}} operator))

    (println "== jurisdiction/assess eng-1 (escalates -- human approves; EIS operator claim distinguishes IUB/VDAA) ==")
    ...
    (println "== jurisdiction/assess eng-1-again (fuse-platform-operator? -> HARD hold, flagship IUB/VDAA check) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up ur-registration-missing) ==")
    ...
    (println "== filing/submit eng-4 (ur-registration-missing -> HARD hold) ==")
